resource "aws_docdb_cluster" "docdb" {
  ...
  master_username         = jsondecode(data.aws_secretsmanager_secret_version.roboshop.secret_string)["DOCUMENTDB_USERNAME"]
  master_password         = jsondecode(data.aws_secretsmanager_secret_version.roboshop.secret_string)["DOCUMENTDB_PASSWORD"]
